...
int json_to_rdata(uint8_t type, char *data, uint8_t *pos, uint8_t *end,
  ...
  case ns_t_aaaa: {
    ...
    if (pos + r > end)
      return -1;
    if (!inet_pton(AF_INET6, data, pos))
      return -1;
    pos += r;
    break;
  }
  ...
  case ns_t_soa: {
    char *saveptr = NULL;
    int r = ns_name_compress(strtok_r(data, " ", &saveptr), pos, end - pos,
                             dnptrs, lastdnptr);
    if (r < 0) {
    ...
    pos += r;
    r = ns_name_compress(strtok_r(NULL, " ", &saveptr), pos, end - pos, dnptrs,
                         lastdnptr);
    if (r < 0) {
...
int json_to_dns(uint16_t tx_id, char *in, uint8_t *out, int olen) {
  ...
  for (int i = 0; i < obj->length; i++) {
    ...
    int r = ns_name_compress(nx_json_get(subobj, "name")->text_value, pos,
